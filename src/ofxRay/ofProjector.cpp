...
namespace ofxRay {
	...
	ofRay Projector::castPixel(int x, int y) const {
		return castPixel(ofVec2f(x, y));	
	}

	ofRay Projector::castPixel(const ofVec2f& xy) const {
		return castCoordinate(ofVec2f(	(xy.x + 0.5f)/ (float) width * 2.0f - 1.0f,
										1.0f - (xy.y + 0.5f) / (float) height * 2.0f)	);
	}

	void Projector::castPixels(const vector<ofVec2f>& xy, vector<ofRay>& rays) const {
	...
	ofRay Projector::castCoordinate(const ofVec2f& xy) const {
	...
	void Projector::castCoordinates(const vector<ofVec2f>& xy, vector<ofRay>& rays) const {
		ofMatrix4x4 matrix = this->getProjectionMatrix();
		matrix.preMult(this->getViewMatrix());
